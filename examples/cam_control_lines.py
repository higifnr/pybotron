...
def chaumette_projection(cam : Camera, line : PluckerLine):
        l_cam = line.transform(inv(cam.pose))
        m = l_cam.m 
        a1,b1,c1 = m
        r = (a1**2 + b1**2)**0.5
        c = a1/r; s = b1/r
...
s_d = np.hstack([chaumette_projection(cam_d, l1),chaumette_projection(cam_d, l2)])
...
def update(frame):
    ...
    s_c= np.hstack([chaumette_projection(cam_c, l1),chaumette_projection(cam_c, l2)])
